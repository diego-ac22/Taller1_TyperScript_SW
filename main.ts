...
let tablaSerie: HTMLElement = document.getElementById("serie")!;
...
function mostrarSerie(serie: Serie):void
{
    ...
    for(let ser of serie)
    {
        let tablaSerie:HTMLElement = document.createElement("serie");
        tablaSerie.innerHTML= `<td>${serie.id}</td>
        <td>${serie.nombres}</td>
        <td>${serie.canales}</td>
        <td>${serie.temporadas}</td>`;
        tbodySerie.appendChild(tablaSerie);
    }
}